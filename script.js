window.addEventListener('load', () => {
    ...
    function updateScroll() {
        ...
        if (progress <= 0.35) {
            ...
            if (introContainer) {
                introContainer.style.opacity = window.innerWidth <= 768 ? Math.max(0, 1 - p4) : 1;
            }
        }

        // --- B. 가로 슬라이드 및 화면 잠금 제어 구간 ---
        const afterIntroScroll = scrollTop - introScroll;

        if (afterIntroScroll <= 0 || (afterIntroScroll > 0 && afterIntroScroll <= pauseScroll)) {
    ...
    // [2] Work Page 데이터 바인딩 및 Sanity 제어 구간
    const createClient = window.createClient || window.sanityClient;
    ...
    if (createClient && imageUrlBuilder) {
        ...
        function openProjectDetail(project) {
            ...
            detailWrap.classList.remove('hidden'); // 클릭 시 모달창이 보이도록 hidden 제거 기능 복구
            detailTitle.textContent = project.title || '';
            detailCategory.textContent = project.category || '';
            detailDescription.textContent = project.description || '';

            const detailImageData = project.contentImages || project.detailImages || project.images || project.gallery || project.detailImage || [];
